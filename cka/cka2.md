# CKA Mock Exam – Rapid Memorization Cheat Sheet (Mock 2)

---

## Q1 – StorageClass + Static PV + PVC
**Context:** Create local static storage (SC → PV → PVC).
...
## Q2 – Find highest CPU node across clusters
...
## Q4 – Helm upgrade (validate → install → uninstall)
...
## Q5 – Deployment blocked by ResourceQuota
...
## Q7 – Ingress 503 due to wrong protocol
...
## Q10 – App fails due to missing Secret
...
## Q11 – CrashLoop due to ConfigMap mount
...
## Q12 – Ingress 404 (wrong host/service/port)
...
**Context:** /api → api-service, / → web-service.
...
## Q16 – kubectl broken (API server down)
**Context:** kubelet missing → API server not running.
...
## 🔑 Exam Pattern Hooks
- PVC not binding → **nodeAffinity / SC mismatch**
- Pod Pending → **Quota / Scheduler**
- 503 ingress → **Service protocol**
